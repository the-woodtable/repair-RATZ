...
void setup() {
  Serial.begin(460800);   // MUST equal CAM_BAUD in StereoCameras.h.
                          // Change one, change the other, reflash BOTH boards.
  ...
  // Frame buffers: 2 buffers in PSRAM give the smoothest stream, but they
  // REQUIRE PSRAM. Without it, allocation quietly falls short -> truncated
  // (grey-bottom) frames and the same stale buffer returned forever.
  if (psramFound()) {
    config.jpeg_quality = 15;             // ~8-15 KB/frame
    config.fb_count     = 2;
    config.fb_location  = CAMERA_FB_IN_PSRAM;
    config.grab_mode    = CAMERA_GRAB_LATEST;
  } else {
    config.jpeg_quality = 18;             // smaller frames to fit DRAM
    config.fb_count     = 1;
